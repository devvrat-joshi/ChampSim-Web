...
void v4_initialize_branch_predictor(){
    int cpu = 0;
    current_cpu=cpu;
...
//*******************************************************************PREDICT BRANCH*********************************************************
uint8_t v4_predict_branch(uint64_t pc){
    int cpu = 0;
...
//*******************************************************************LAST BRANCH RESULT********************************************************
void v4_last_branch_result(uint64_t pc, uint8_t taken,uint64_t branch_target){
    int cpu = 0;
...
void O3_CPU::initialize_branch_predictor(){
    ...
    v4_initialize_branch_predictor();
}

uint8_t O3_CPU::predict_branch(uint64_t ip){
    uint8_t prediction;
    gshare = gshare_predict_branch(ip);
    v4 = v4_predict_branch(ip);
...
void O3_CPU::last_branch_result(uint64_t ip, uint8_t taken, uint64_t target)
{
    ...
    v4_last_branch_result(ip,taken,target);
}